...
data_for_train = torch.load('./data/datalist_detection_train.pt')
data_for_train_loader = DataLoader(data_for_train, batch_size=128)
data_for_test = torch.load('./data/datalist_detection_test.pt')
data_for_test_loader = DataLoader(data_for_test, batch_size=128)
...
Benign_weight = 1
Malicious_weight = Benign / Malware




class_weight = torch.FloatTensor([Benign_weight,
                                  Malicious_weight]).cuda()
...
# train model for IoT malware detection
def train(epoch, loader):
    ...
    for data in loader:
        ...
        optimizer.step()
    return loss_all / len(data_for_train)
...
for epoch in tqdm(range(1, 401)):
    ...
    loss = train(epoch, data_for_train_loader)
    con, acc, precision, recall, f1 = test(data_for_train_loader)
    print(
    ...
    con, acc, precision, recall, f1 = test(data_for_test_loader)
